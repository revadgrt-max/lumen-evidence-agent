...
(() => {
  ...
  function renderMetrics(sc) {
    const hr = sc && sc.hallucinationRate != null ? (sc.hallucinationRate * 100).toFixed(0) + '%' : '0%';
    ...
    $('metrics').innerHTML = `
      ...
      <div class="metric"><div class="n ok">${hr}</div><div class="l">hallucination rate</div></div>
      <div class="metric"><div class="n">${flagCount}</div><div class="l">flagged, not bluffed</div></div>`;
    $('hrate').textContent = hr;
  }
  function renderRoi() {
